...
def index():
    ...
    return render_template(f"{template}.html", hub=hub, negocios=negocios, categorias=categorias)
...
def pagina_filtro(segmento):
    ...
    if hub["tipo"] == "bairro":
        # segmento é um bairro → lista negócios desse bairro
        negocios = query("""
            SELECT n.*, c.nome as categoria_nome, c.slug as categoria_slug
            FROM hub_negocios n
            JOIN hub_negocio_hubs nh ON nh.negocio_id = n.id
            JOIN hub_categorias c ON c.id = n.categoria_id
            WHERE nh.hub_id = %s AND n.ativo = true
              AND LOWER(n.bairro) = LOWER(%s)
            ORDER BY n.nome
        """, (hub["id"], segmento))
        filtro_tipo = "bairro"
        filtro_valor = segmento
    else:
        # segmento é uma categoria → lista negócios dessa categoria
        negocios = query("""
            SELECT n.*, c.nome as categoria_nome, c.slug as categoria_slug
            FROM hub_negocios n
            JOIN hub_negocio_hubs nh ON nh.negocio_id = n.id
    ...
    return render_template(f"{template}.html", hub=hub, negocios=negocios,
...
def pagina_negocio(segmento, slug_negocio):
    ...
    return render_template(f"{template}.html", hub=hub, negocio=negocio, segmento=segmento)
...
@app.route("/admin/hubs/novo", methods=["GET", "POST"])
@login_required
def admin_hub_novo():
    if request.method == "POST":
        d = request.form
        ...
        """, (
            ...
            d["nome"], d["slug"],
            d.get("dominio_proprio") or None,
            d.get("hub_leanttro") or None,
            d.get("tipo", "bairro"),
            d.get("bairro_fixo") or None,
            d.get("categoria_fixa") or None,
            d.get("logo_url") or None,
            d.get("cor_primaria", "#7943e2"),
            d.get("cor_secundaria", "#5c2ec2"),
            d.get("titulo") or None,
            d.get("descricao") or None,
            d.get("ga4_id") or None,
            d.get("pixel_id") or None,
            d.get("instagram_url") or None,
            d.get("whatsapp") or None,
    ...
    usuarios = query("SELECT id, nome, email FROM usuarios ORDER BY nome")
    return render_template("admin/hub_form.html", hub=None, usuarios=usuarios)
...
def admin_hub_editar(hub_id):
    ...
    if request.method == "POST":
        ...
        return redirect("/admin/hubs")
    usuarios = query("SELECT id, nome, email FROM usuarios ORDER BY nome")
    return render_template("admin/hub_form.html", hub=hub, usuarios=usuarios)
...
@app.route("/admin/negocios/novo", methods=["GET", "POST"])
@login_required
def admin_negocio_novo():
    if request.method == "POST":
        ...
        """, (
            ...
            d.get("site_url") or None,
            "mostrar_foto"      in d,
            "mostrar_descricao" in d,
            "mostrar_whatsapp"  in d,
            "mostrar_instagram" in d,
            "mostrar_telefone"  in d,
            "mostrar_site"      in d,
            "mostrar_endereco"  in d,
            "mostrar_mapa"      in d,
            "ativo"             in d,
        ))
        ...
        # Vincula aos hubs selecionados
        hubs_sel = request.form.getlist("hubs")
        for hub_id in hubs_sel:
            query("""
    ...
    categorias = query("SELECT * FROM hub_categorias WHERE ativo = true ORDER BY nome")
    hubs = query("SELECT id, nome, slug FROM hub_clientes WHERE ativo = true ORDER BY nome")
    return render_template("admin/negocio_form.html", negocio=None,
                           categorias=categorias, hubs=hubs, hubs_do_negocio=[])
...
def admin_negocio_editar(negocio_id):
    ...
    return render_template("admin/negocio_form.html", negocio=negocio,
                           categorias=categorias, hubs=hubs,
                           hubs_do_negocio=hubs_do_negocio)
...
def admin_usuarios():
    usuarios = query("""
        SELECT u.*, a.nivel as admin_nivel
        FROM usuarios u
...
def admin_usuario_novo():
    if request.method == "POST":
        ...
        if d.get("nivel"):
            cur.execute("INSERT INTO admins (user_id, nivel) VALUES (%s, %s)",
                        (user_id, d["nivel"]))
        get_db().commit()
...
def admin_usuario_editar(user_id):
    ...
    if request.method == "POST":
        ...
        if d.get("nivel"):
            query("INSERT INTO admins (user_id, nivel) VALUES (%s, %s)",
                  (user_id, d["nivel"]), commit=True)
        return redirect("/admin/usuarios")
    return render_template("admin/usuario_form.html", usuario=usuario, admin=admin)
...
def admin_assinaturas():
    ...
    """)
    return render_template("admin/assinaturas.html", assinaturas=assinaturas)


@app.route("/admin/assinaturas/nova", methods=["GET", "POST"])
@login_required
def admin_assinatura_nova():
    if request.method == "POST":
        ...
        """, (
            ...
            d.get("valido_ate") or None,
            d.get("mp_sub_id") or None
        ), commit=True)
        return redirect("/admin/assinaturas")
    usuarios = query("SELECT id, nome, email FROM usuarios ORDER BY nome")
    return render_template("admin/assinatura_form.html", assinatura=None, usuarios=usuarios)
...
def admin_assinatura_editar(ass_id):
    ...
    if request.method == "POST":
        ...
        return redirect("/admin/assinaturas")
    usuarios = query("SELECT id, nome, email FROM usuarios ORDER BY nome")
    return render_template("admin/assinatura_form.html", assinatura=ass, usuarios=usuarios)
...
# ════════════════════════════════════════════════════════════
#  API — endpoints JSON (opcionais, pra uso futuro)
# ════════════════════════════════════════════════════════════
